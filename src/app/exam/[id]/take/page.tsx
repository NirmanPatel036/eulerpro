...
function calcResults(questions: Question[], answers: Answers) {
  ...
  const questionResults = questions.map((q) => {
    ...
    if (q.type === "multiple_choice")
      correct = ans === q.answer_data.correct_option;
    else if (q.type === "true_false")
      correct = ans === q.answer_data.correct_answer;
    else if (q.type === "fill_blank") {
      const regex = q.answer_data.answer_regex as string | undefined;
      correct = regex
        ? new RegExp(regex, "i").test((ans as string) ?? "")
        : false;
      ...
      >
        ...
        <div className="px-5 -mt-4 mb-4">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm grid grid-cols-3 divide-x divide-gray-100 text-center">
            ...
            <div className="py-3 px-2">
              <p
                className={`text-lg font-bold ${unanswered > 0 ? "text-red-500" : "text-gray-300"}`}
              >
                {unanswered}
              </p>
            ...
            <div className="py-3 px-2">
              <p
                className={`text-lg font-bold ${flaggedCount > 0 ? "text-amber-500" : "text-gray-300"}`}
              >
                {flaggedCount}
              </p>
...
export default function ExamTakePage() {
  ...
  const analyzeLoopRef = useRef<ReturnType<typeof setInterval> | undefined>(
    undefined,
  );
  ...
  const reportDiscreteFlag = useCallback(
    async (flagType: "tab_switch" | "copy_paste", description: string) => {
      if (!sessionId) return;
      try {
  ...
  const analyzeCurrentFrame = useCallback(async () => {
    ...
    try {
      ...
      const resp = await fetch(
        `${backendBaseUrl}/api/v1/sessions/${sessionId}/analyze-frame`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (!resp.ok) return;
      const analysis = (await resp.json()) as {
        flags?: Array<{
          type?: string;
          severity?: "high" | "medium" | "low";
          description?: string;
  ...
  useEffect(() => {
    (async () => {
      ...
      // Create or get exam session using upsert to avoid race conditions
      // The unique constraint is (exam_id, student_id)
      ...
      if (upsertResult.error) {
        console.error(
          "[ExamTakePage] Failed to create/get exam session",
          upsertResult.error,
        );
        router.push("/dashboard/student/exams");
        return;
      ...
      if (!upsertResult.data || !upsertResult.data.id) {
        console.error("[ExamTakePage] Session upsert returned no data");
        router.push("/dashboard/student/exams");
        return;
      }

      const sessionId_temp = upsertResult.data.id;

      // Now fetch exam + questions with retry logic
      let ex, exError, qs, qsError;
      let retries = 0;
      const maxRetries = 3;
      ...
      while (retries < maxRetries) {
        const examResult = await supabase
          ...
          .single();
        ex = examResult.data;
        exError = examResult.error;

        const questionsResult = await supabase
          .from("questions")
          .select(
            'id, type, text, points, difficulty, "order", media_url, media_type, answer_data',
          )
          .eq("exam_id", examId)
          .order('"order"');
        qs = questionsResult.data;
        qsError = questionsResult.error;
        ...
        // If we got exam but not questions, wait a moment for RLS to catch up
        if (ex && !qs && retries < maxRetries - 1) {
          console.warn(
            "[ExamTakePage] Questions fetch failed, retrying after delay...",
            { qsError, retries },
          );
      ...
      if (!ex || !qs) {
        console.error("[ExamTakePage] Failed to load exam data after retries", {
          exError,
          qsError,
          examId,
          retries,
      ...
      const examData: ExamData = {
        ...
        questions: (qs as Question[]).map((q) => ({
          ...
          media_type:
            q.media_type === "video" || q.media_type === "image"
              ? q.media_type
              : null,
          options: (q.answer_data as Record<string, unknown>).options as
            | string[]
            | undefined,
        })),
      };
      ...
      // (We already checked/created the session before fetching questions)
      {
        // Fetch full session data now that we know it exists
        const fullSessionResult = await supabase
          .from("exam_sessions")
          .select("id, answers, status, completed_at, started_at")
          .eq("id", sessionId_temp)
          .single();

        if (fullSessionResult.error || !fullSessionResult.data) {
          console.error(
            "[ExamTakePage] Failed to fetch full session data",
            fullSessionResult.error,
          );
          router.push("/dashboard/student/exams");
          return;
        }

        const existing = fullSessionResult.data;
        ...
        if (existing.status === "active") {
          ...
          // Session exists but is completed, check if exam was rescheduled
          const scheduledAtMs = ex.scheduled_at
            ? new Date(ex.scheduled_at).getTime()
            : null;
          const completedAtMs = existing.completed_at
          ...
          if (isRescheduledAfterCompletion) {
            // Reset the session for the rescheduled exam
            const nowIso = new Date().toISOString();
            ...
            if (resetErr) {
              console.error(
                "[ExamTakePage] Failed to reset session for rescheduled exam",
                resetErr,
              );
            ...
            // Already completed and not rescheduled
            router.push(`/exam/${examId}/results`);
            return;
          }
        } else {
          // Some other status - redirect to safety
          console.error(
            "[ExamTakePage] Unexpected session status",
            existing.status,
          );
          router.push("/dashboard/student/exams");
          return;
  ...
  const handleConfirmSubmit = useCallback(async () => {
    ...
    const { score, maxScore, questionResults } = calcResults(
      exam.questions,
      answers,
    );
    ...
    <div className="h-screen bg-[#f8f8fc] flex flex-col overflow-hidden">
      ...
      <header className="h-16 bg-white border-b border-gray-100 px-6 flex items-center justify-between shrink-0 z-20 shadow-sm">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <h1 className="font-bold text-gray-900 text-sm truncate max-w-xs hidden sm:block">
            {exam.title}
          </h1>
        ...
        <div className="flex-1 max-w-md mx-8 hidden md:flex flex-col gap-1.5">
          ...
          <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
            <div
              className="h-full bg-[#4b3fe9] rounded-full transition-all duration-500"
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div
            className={cn(
              "flex items-center gap-1.5 px-3 py-1.5 rounded-lg border font-mono font-bold text-sm tabular-nums",
              timeBg,
            )}
          >
                    ...
                    <div className="px-8 py-6 border-b border-gray-50 bg-gray-50/40 flex items-start justify-between gap-4 shrink-0">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex items-center gap-2 mb-2.5 flex-wrap">
                          ...
                          <Badge
                            className={cn(
                              "border-0 text-[11px] capitalize px-2.5 py-0.5",
                              DIFF_CLS[currentQ.difficulty] ??
                      ...
                      >
                        <Flag
                          className="w-4 h-4"
                          fill={
                            flagged.has(currentQ.id) ? "currentColor" : "none"
                          }
                        />
                      </button>
                    </div>
                        ...
                        {currentQ.media_url &&
                          (currentQ.media_type === "video" ? (
                            ...
                            <video
                              ...
                              controls
                              className="mt-3 rounded-xl max-h-80 w-full object-contain bg-white border border-gray-200"
                            />
                          ) : (
                            ...
                            <label
                              ...
                              className={cn(
                                ...
                                answers[currentQ.id] === i
                                  ? "border-[#4b3fe9] bg-[#4b3fe9]/5 shadow-sm"
                          ...
                          ["True", "False"].map((v, i) => (
                            ...
                            >
                              ...
                              <input
                                type="radio"
                                name={`q-${currentQ.id}`}
                                checked={answers[currentQ.id] === (i === 0)}
                                onChange={() => setAnswer(i === 0)}
                                className="sr-only"
                              />
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={toggleFlag}
                    className={cn(
                      "hidden sm:flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all",
                      flagged.has(currentQ.id)
                  ...
                  >
                    <Bookmark
                      className="w-4 h-4"
                      fill={flagged.has(currentQ.id) ? "currentColor" : "none"}
                    />
          ...
          <div className="flex-1 overflow-y-auto p-5">
            ...
            <div className="flex flex-wrap gap-x-3 gap-y-2 mb-4 text-[11px] text-gray-400">
              <div className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-[#4b3fe9]" />
                Current
              </div>
              <div className="flex items-center gap-1.5">
