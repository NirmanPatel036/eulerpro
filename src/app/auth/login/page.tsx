...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Attempt OTP with shouldCreateUser: false — Supabase will error if the
        // user does not exist, letting us redirect to sign-up gracefully.
        ...
        if (err) {
            // Rate-limit or server errors → show inline
            if (err.status === 429 || err.message.toLowerCase().includes('rate limit')) {
                setError(err.message);
                return;
            }
            // Otherwise the email isn\'t registered → send them to sign-up
            router.push(
                `/auth/register?email=${encodeURIComponent(email)}&hint=no-account`
            );
            return;
        }
    ...
    return (
        <div className="auth-shell">
            ...
            <div className="auth-card">
                ...
                <p className="auth-subheading">Enter your credentials to access your exam dashboard</p>
                ...
                {sent && <div className="auth-success">Check your email — we sent a magic link!</div>}
                ...
                <div className="auth-socials">
                    <button className="social-btn" type="button" aria-label="Continue with Google" onClick={handleGoogle}>
                    ...
                    <button className="social-btn" type="button" aria-label="Continue with Apple" disabled style={{ opacity: 0.4, cursor: 'not-allowed' }}>
                    ...
                    <button className="social-btn" type="button" aria-label="Continue with Microsoft" disabled style={{ opacity: 0.4, cursor: 'not-allowed' }}>
                        <svg width="20" height="20" viewBox="0 0 24 24">
                ...
                <p className="auth-footer-text">No account? <Link href="/auth/register">Create one</Link></p>
