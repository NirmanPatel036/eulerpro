...
function rowToQuestion(row: Record<string, unknown>): Question {
    const mediaUrl = typeof row.media_url === 'string' ? row.media_url : undefined;
    ...
    switch (type) {
        case 'multiple_choice':
            return { ...base, type, options: (ans.options as string[]) ?? [], correct_option: (ans.correct_option as number) ?? 0 };
        case 'checkbox':
            return { ...base, type, options: (ans.options as string[]) ?? [], correct_options: (ans.correct_options as number[]) ?? [] };
        case 'true_false':
            return { ...base, type, correct_answer: (ans.correct_answer as boolean) ?? true };
        case 'fill_blank':
            return { ...base, type, answer_regex: (ans.answer_regex as string) ?? '.*', sample_answer: (ans.sample_answer as string) ?? '' };
        case 'matching':
            return { ...base, type, pairs: (ans.pairs as { left: string; right: string }[]) ?? [] };
        case 'reorder':
            return { ...base, type, items: (ans.items as string[]) ?? [], correct_order: (ans.correct_order as number[]) ?? [] };
        default:
            return { ...base, type: 'multiple_choice', options: [], correct_option: 0 };
    }
}
...
const PROCTORING_OPTS = [
    ...
    { id: 'fullscreen', label: 'Fullscreen lock',      desc: 'Prevent exiting exam window' },
...
export default function EditExamPage() {
    ...
    /* proctoring */
    const [proctoring, setProctoring] = useState<string[]>(['faceid', 'tabswitch', 'fullscreen']);
    const toggleProc = (id: string) =>
        setProctoring(p => p.includes(id) ? p.filter(x => x !== id) : [...p, id]);
    ...
    useEffect(() => {
        ...
        async function loadExam() {
            ...
            try {
                ...
                const loadedQs = (qRows ?? []).map((r: Record<string, unknown>) => rowToQuestion(r));
                ...
                setFetchError(err instanceof Error ? err.message : 'Failed to load exam.');
            } finally {
                setLoading(false);
            }
        }
    ...
    const handleSave = async (isDraft: boolean) => {
        ...
        try {
            ...
            const removedIds  = [...originalQIds.current].filter(id => !currentIds.has(id));
            ...
            const buildRow = async (q: Question) => {
                ...
                const { id, type, text, points, difficulty,
    ...
    const handlePublish = async () => {
        ...
        setPublishOk(`✓ Published — ${notified} of ${enrolled} student${enrolled !== 1 ? 's' : ''} notified`);
    };
    ...
    if (loading) {
        return (
            <div className="instructor-home flex items-center justify-center" style={{ minHeight: 'calc(100vh - 120px)' }}>
                    ...
                    <div className="flex-1 min-w-0 overflow-y-auto bg-white rounded-xl border border-gray-200">
                        {selectedQ ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center text-center p-8">
                                ...
                                <p className="text-xs text-gray-300 mt-1">Pick a question from the list or add a new one</p>
                                <Button
                                    onClick={() => setShowTypeModal(true)}
                                    variant="outline"
                    ...
                    <div className="w-72 shrink-0 overflow-y-auto flex flex-col gap-3">
                            ...
                            <div className="space-y-2.5">
                                {PROCTORING_OPTS.map(opt => (
                                    <label key={opt.id} className="flex items-start gap-2.5 cursor-pointer">
                                        ...
                                        />
                                        <div>
                                            <p className={cn('text-xs font-medium', proctoring.includes(opt.id) ? 'text-gray-800' : 'text-gray-500')}>
                                                {opt.label}
                                            </p>
                        ...
                        <div className="bg-gray-50 rounded-xl border border-gray-100 p-4">
                            <div className="grid grid-cols-2 gap-y-2 text-xs">
                                ...
                                <span className="font-semibold text-gray-800 text-right">{proctoring.length} rules</span>
                            </div>
                        </div>
                    </div>
                </motion.div>
