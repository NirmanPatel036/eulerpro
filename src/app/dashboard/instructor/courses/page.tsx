...
export default function InstructorCoursesPage() {
  ...
  const fetchCourses = useCallback(async () => {
    ...
    const { data, error: err } = await supabase
      .from("courses")
      .select(
        "id, name, code, description, cover_image_url, created_at, course_enrollments(count)",
      )
      ...
      {loading && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {[1, 2, 3, 4].map((n) => (
            <div
              key={n}
              className="bg-white rounded-2xl border border-gray-200 overflow-hidden flex flex-col animate-pulse"
          ...
          {courses.length === 0 && (
            ...
            >
              ...
              <p className="text-xs text-gray-300">
                Click &ldquo;New Course&rdquo; to get started
              </p>
            </motion.div>
          )}
        ...
        <DialogContent className="max-w-sm rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm text-gray-500 mt-1">
              ...
              and all its enrollment records will be permanently deleted. This
              cannot be undone.
            </DialogDescription>
          </DialogHeader>
